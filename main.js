...
async function test() {
  ...
  for (let i = 0; i < urls.length; i++) {
    promises.push(downloadMetaSomething(urls[i]));
  }

  await Promise.all(promises)
  .then( result => {
    let data = [];
    result.forEach( a => { data.push(a)});
...
function downloadMetaSomething(url) {
  console.log(url);
  return new Promise( (resolve, reject) => {
    https.get(url, (resp) => {
      ...
      resp.on('end', () => {
        ...
        let songName = document.getElementsByClassName('title_wrap')[0].children[2].innerHTML;

        // Get Authors
        const childNodes = document.getElementsByClassName('title_wrap')[0].children[1].children;
        let artists = [];
        for (let i = 0; i < childNodes.length; i++) {
         let artistId = childNodes[i].href;
         let artist = childNodes[i].children[0].innerHTML;
         artists.push( { artist, artistId } );
        }
        let songId = url;
        resolve({ artists, songName, songId } );
      });
